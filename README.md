...
|  이름  | 역할 | 담당 기능 |
|---|---|---|
| **[@shinae1023](https://github.com/shinae1023)** | 백엔드 | **최종 재판(3차)** 로직 (AI 판결문 생성, 판결 아카이브), **의견 채택** 및 자동 채택/판결 스케줄러,<br> **마이페이지** (업적, 경험치 히스토리, 등급 관리), **메인페이지** (실시간 랭킹),<br> **실시간 알림(SSE)**, 좋아요 기능, CI/CD 및 인프라(S3, Swagger) 설정 |
| **[@whc9999](https://github.com/whc9999)** | 백엔드 | **인증(Auth)** (회원가입, 이메일 인증, JWT/Security),<br> **사건 생성(1차)** (Solo/VS 모드, AI 초심 판결, 매칭 대기열),<br> **토론(2차)** (변론/반론 CRUD, 계층형 댓글, 배심원 투표),<br> **신고 시스템** (사용자 신고 접수 및 **관리자(Admin)** 허위 신고 관리) |
