...
### 3\. 게이미피케이션 (MyPage)
  ...
  * **랭킹 시스템**: `Redis ZSet`을 활용하여 실시간으로 '핫한 토론'을 산정합니다.
  * **업적 & 등급**: 활동량(승소, 댓글 수 등)에 따라 경험치(EXP)를 획득하고, '변호사', '판사' 등으로 등급이 승격됩니다.

### 4\. 운영 및 모니터링

  * **신고 자동화**: 욕설/비방 등 부적절한 변/반론이 누적 신고(3회)되면 자동으로 블라인드 처리되고 Slack으로 알림이 전송됩니다.
  * **건의 사항** : 서비스에 대한 건의사항을 제안하고 slack을 통해 운영자가 확인할 수 있습니다.
  * **스케줄러 시스템**:
    * AdoptAutoScheduler: 투표 마감된 사건의 인기 의견 자동 채택
    * JudgmentUpdateScheduler: 매일 자정 최종심 진행 상황 스냅샷 저장 (판결 히스토리)
    * RankingScheduler: 랭킹 점수 시간대별 감쇠(Decay) 적용으로 최신 트렌드 반영
...
### 🔔 알림 및 신고
  ...
  - **신고 기능** → 부적절한 콘텐츠 신고시 Slack 알림 전송, 3회 누적 시 자동 블라인드
...
|  이름  | 역할 | 담당 기능 |
|---|---|---|
| **[@shinae1023](https://github.com/shinae1023)** | 백엔드 | **최종 재판(3차)** 로직 (AI 판결문 생성, 판결 아카이브), **의견 채택** 및 자동 채택/판결 스케줄러,<br> **마이페이지** (업적, 경험치 히스토리, 등급 관리, 참여 재판), **메인페이지** (실시간 랭킹),<br> **실시간 알림(SSE)**, 좋아요 기능, CI/CD 및 인프라(S3, Swagger) 설정 |
| **[@whc9999](https://github.com/whc9999)** | 백엔드 | **인증(Auth)** (회원가입, 이메일 인증, JWT/Security),<br> **사건 생성(1차)** (Solo/VS 모드, AI 초심 판결, 매칭 대기열),<br> **토론(2차)** (변론/반론 CRUD, 계층형 댓글, 배심원 투표),<br> **신고 시스템** (사용자 신고 접수 및 **관리자(Admin)** 허위 신고 관리), **건의 시스템** (사용자 건의 사항 접수) |
